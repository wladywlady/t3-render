...
function hasOverlap(terms, text) {
    if (terms.length === 0)
        return true;
...
chatRouter.post("/", async (req, res, next) => {
    try {
        ...
        const relevantChunks = chunks.filter((chunk) => normalizeScore(chunk.score) >= MIN_NORMALIZED_SCORE && hasOverlap(questionTerms, chunk.text));
